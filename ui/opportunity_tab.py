...
def render(dados_df: pd.DataFrame):
    key = st.text_input(C.UI_LABEL_ACCESS_KEY,
                        type="password", key="opp_access_key")
    if key != API_KEY:
    ...
    with tabs[0]:
        ...
        min_pop = st.slider(
            ...
            value=min(20000, int(df["pop_2022"].max()
                      if not df.empty else 1000000)),
        )
        ...
        if ranked.empty:
            ...
            geo_rows = []
            for _, row in (
                ranked.sort_values("score", ascending=False).head(
                    top_n).iterrows()
            ):
            ...
            if geo_rows:
                ...
                fig = px.scatter_mapbox(
                    ...
                    zoom=3,
                    center={"lat": C.MAP_LAT_DEFAULT,
                            "lon": C.MAP_LON_DEFAULT},
    ...
    with tabs[1]:
        ...
        if st.button(C.UI_LABEL_EXECUTE_ANALYSIS, key="btn_det"):
            with st.spinner(C.UI_LABEL_COLLECTING_INDICATORS):
                ...
                inds = get_unidades_locais(
                    base["id"].astype(str).tolist(), "all")
                det = base.merge(inds, on="id", how="left")

            if det.empty:
                st.info(C.UI_LABEL_NO_DATA_SUFFICIENT)
            else:
                det["unidades_locais"] = det["unidades_locais"].fillna(
                    0).astype(int)
                ...
                det["score_area"] = w_emp * \
                    det["emp_norm"] + w_pop * det["pop_norm"]
                ...
                st.plotly_chart(
                    px.bar(
                        det.sort_values(
                            "score_area", ascending=False).head(30),
                        x="nome",
                        y="score_area",
                        color="uf",
    ...
    with tabs[2]:
        ...
        with c1:
            selected_area = st.selectbox(
                C.UI_LABEL_SELECT_AREA, list(C.COURSES.keys()))
        with c2:
            available_courses = C.COURSES.get(selected_area, [])
            selected_course = st.selectbox(
                C.UI_LABEL_SELECT_COURSE, available_courses)
        ...
        if st.button(C.UI_LABEL_ANALYZE_POTENTIAL):
            with st.spinner(
                C.UI_LABEL_ANALYZING_MARKET.format(
                    course=selected_course, area=selected_area)
            ):
                ...
                ranked_course = (
                    final.sort_values(
                        "score_curso", ascending=False).head(50).copy()
                )
                ...
                st.dataframe(ranked_course[[
                             "nome", "uf", "pop_2022", "unidades_locais", "score_curso"]].reset_index(drop=True))
    ...
    with tabs[3]:
        ...
        with col1:
            # Parameters
            eps_km = st.slider(C.UI_LABEL_EPS_KM, min_value=10,
                               max_value=200, value=50, step=10)
            min_samples = st.slider(
                C.UI_LABEL_MIN_SAMPLES, min_value=2, max_value=10, value=3)
        with col2:
            ufs_all = sorted(list(C.ESTADO_REGIAO.keys()))
        ...
        if st.button(C.UI_LABEL_RUN_CLUSTERING):
            with st.spinner("Executando DBSCAN..."):
                ...
                mask = (base["presenca"] == 0) & (
                    base["pop_2022"] > 10000)  # Filter small villages
                candidates = base[mask].copy()

                # Sort by pop to prioritize bigger opportunities
                candidates = candidates.sort_values(
                    "pop_2022", ascending=False).head(200)

                if candidates.empty:
                    st.warning(
                        "Nenhuma cidade candidata encontrada com os filtros atuais.")
                else:
                    # Geocode
                    coords = []
                    valid_indices = []
                    ...
                    if not coords:
                        st.error(
                            "Não foi possível obter coordenadas para as cidades selecionadas.")
                    else:
                        ...
                        if real_clusters.empty:
                            ...
                            st.success(
                                f"Encontrados {n_clusters} clusters de oportunidade!")
    ...
    with tabs[4]:
        ...
        if st.button("Executar Análise de Regressão"):
            with st.spinner("Calculando modelo estatístico..."):
                ...
                sales_data = dados_df.groupby(
                    [C.COL_INT_CITY, C.COL_INT_STATE]).size().reset_index(name="vendas")
                sales_data["id_match"] = sales_data.apply(
                    lambda x: f"{str(x[C.COL_INT_CITY]).strip().upper()}|{str(x[C.COL_INT_STATE]).strip().upper()}", axis=1)
                ...
                # Create match key
                features["id_match"] = features.apply(
                    lambda x: f"{str(x['nome']).strip().upper()}|{str(x['uf']).strip().upper()}", axis=1)
                ...
                df_reg = pd.merge(sales_data, features,
                                  on="id_match", how="inner")

                if len(df_reg) < 10:
                    st.warning(
                        "Dados insuficientes para regressão (mínimo 10 cidades com vendas).")
                else:
                    # Prep Data
                    df_reg["pop_2022"] = df_reg["pop_2022"].fillna(0)
                    df_reg["unidades_locais"] = df_reg["unidades_locais"].fillna(
                        0)
                    ...
                    c2.metric(C.UI_LABEL_REGRESSION_COEF_POP,
                              f"{model.coef_[0]:.2e}")
                    c3.metric(C.UI_LABEL_REGRESSION_COEF_EMP,
                              f"{model.coef_[1]:.2e}")
                    ...
                    fig = px.scatter(
                        ...
                        y="vendas_previstas",
                        hover_data=["nome", "uf",
                                    "pop_2022", "unidades_locais"],
                        labels={"vendas": "Vendas Reais",
