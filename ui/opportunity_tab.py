...
def _render_overview_tab(
    ...
    if ranked.empty:
        ...
        st.dataframe(
            ranked.sort_values(["uf", "score"], ascending=[True, False]).reset_index(
                drop=True
            )
        )
...
def _render_detailed_tab(
    ...
    if st.button(C.UI_LABEL_EXECUTE_ANALYSIS, key="btn_det"):
        ...
        if det.empty:
            ...
            st.dataframe(
                det.sort_values(["score_area"], ascending=False).reset_index(drop=True)
            )


def _render_course_tab(
    dados_df: pd.DataFrame,
    build_oportunidade_por_uf: Callable[[pd.DataFrame, List[str]], pd.DataFrame],
    get_unidades_locais: Callable[[str, str], int],
) -> None:
    ...
    if st.button(C.UI_LABEL_ANALYZE_POTENTIAL):
        with st.spinner(
            ...
            final["unidades_locais"] = final["unidades_locais"].fillna(0).astype(int)
            ...
            norm_emp = final["unidades_locais"] / max(final["unidades_locais"].max(), 1)
...
def _render_clustering_tab(
    ...
    with col1:
        # Parameters
        eps_km = st.slider(
            C.UI_LABEL_EPS_KM, min_value=10, max_value=200, value=50, step=10
        )
                ...
                if not coords:
                    ...
                    if real_clusters.empty:
                        ...
                        stats = (
                            ...
                            .agg({"nome": "count", "pop_2022": "sum"})
                            .rename(
                                columns={
                                    "nome": "Cidades",
                                    "pop_2022": "População Total",
                                }
                            )
                            .sort_values("População Total", ascending=False)
                        )
                        st.dataframe(stats)


def _render_regression_tab(
    dados_df: pd.DataFrame,
    build_oportunidade_por_uf: Callable[[pd.DataFrame, List[str]], pd.DataFrame],
    get_unidades_locais: Callable[[str, str], int],
) -> None:
    ...
    if st.button("Executar Análise de Regressão"):
        with st.spinner("Calculando modelo estatístico..."):
            ...
            sales_data = (
                dados_df.groupby([C.COL_INT_CITY, C.COL_INT_STATE])
                .size()
                .reset_index(name="vendas")
            )
            sales_data["id_match"] = sales_data.apply(
                lambda x: f"{str(x[C.COL_INT_CITY]).strip().upper()}|{str(x[C.COL_INT_STATE]).strip().upper()}",
                axis=1,
            )
            ...
            features["id_match"] = features.apply(
                lambda x: f"{str(x['nome']).strip().upper()}|{str(x['uf']).strip().upper()}",
                axis=1,
            )
            ...
            if len(df_reg) < 10:
                ...
                st.info(
                    f"O modelo explica {r2*100:.1f}% da variação nas vendas. "
...
def render(
    ...
    # -------------------------------------------------------------------------
    with tabs[0]:
        _render_overview_tab(dados_df, build_oportunidade_por_uf)

    # -------------------------------------------------------------------------
    # TAB 2: Análise Detalhada (Geral - Old Implementation Refined)
    # -------------------------------------------------------------------------
    with tabs[1]:
        _render_detailed_tab(dados_df, build_oportunidade_por_uf, get_unidades_locais)

    # -------------------------------------------------------------------------
    # TAB 3: Análise por Curso (New Feature)
    # -------------------------------------------------------------------------
    with tabs[2]:
        _render_course_tab(dados_df, build_oportunidade_por_uf, get_unidades_locais)

    # -------------------------------------------------------------------------
    # TAB 4: Geo Clustering (DBSCAN)
    # -------------------------------------------------------------------------
    with tabs[3]:
        _render_clustering_tab(dados_df, build_oportunidade_por_uf)

    # -------------------------------------------------------------------------
    # TAB 5: Regression Analysis
    # -------------------------------------------------------------------------
    with tabs[4]:
        _render_regression_tab(dados_df, build_oportunidade_por_uf, get_unidades_locais)
