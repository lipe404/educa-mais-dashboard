...
def render(contracts_df, faturamento_df):
    ...
    with t1:
        ...
        with c1:
            algo = st.selectbox(
                "Algoritmo",
                ["Prophet (Facebook AI)", "Holt-Winters (Sazonal)"],
    ...
    with t2:
        ...
        try:
            ...
            m1, m2 = st.columns(2)
            m1.metric(
                label=f"Faturamento previsto ({horizon_label_f})",
            ...
            fig_f = px.line(
                ...
                y=C.COL_INT_VALOR,
                color="Type",
                title=f"Previsão de Faturamento Diário - {algo_f}",
                color_discrete_map={
                    "Histórico": C.COLOR_PRIMARY,
                    "Previsão": C.COLOR_FORECAST,
                },
            ...
            st.markdown("---")
            daily_f = (
                df_input_f.groupby(df_input_f[C.COL_INT_DATA].dt.date)[C.COL_INT_VALOR]
                .sum()
                .sort_index()
            )
            if len(daily_f) >= 14:
                ...
                trend_pct_f = (
                    ((recent_avg_f - prev_avg_f) / prev_avg_f * 100)
                    if prev_avg_f > 0
                    else 0
                )
                ...
                txt += (
                    f"- **Média diária esperada:** R$ {future_daily_avg_f:,.2f}/dia\n\n"
                )
                ...
                st.info(
                    "Dados insuficientes para análise detalhada (mínimo 2 semanas)."
                )
        except Exception as e:
            st.error(f"Erro ao gerar previsão: {e}")
            if "não instalada" in str(e):
