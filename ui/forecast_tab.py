...
def render(contracts_df, faturamento_df):
    ...
    with t1:
        ...
        # Backtesting Button
        run_bt = st.button(
            "🧪 Rodar Backtest (Validar Precisão)", key="bt_contracts")
        ...
        if run_bt:
            ...
            try:
                ...
                if "error" in bt_results:
                    ...
                    b1.metric("MAE (Erro Médio Absoluto)",
                              f"{bt_results['mae']:.2f}")
                    b2.metric("RMSE (Raiz do Erro Quadrático)",
                              f"{bt_results['rmse']:.2f}")
                    b3.metric("MAPE (Erro % Médio)",
                              f"{bt_results['mape']:.2f}%")
                    ...
                    fig_bt.add_scatter(x=comp_df[C.COL_INT_DT], y=comp_df[f"{C.UI_LABEL_CONTRACTS}_actual"], name="Realizado", line=dict(
                        color=C.COLOR_PRIMARY))
                    fig_bt.add_scatter(x=comp_df[C.COL_INT_DT], y=comp_df[f"{C.UI_LABEL_CONTRACTS}_predicted"], name="Previsto (Backtest)", line=dict(
                        color=C.COLOR_SECONDARY, dash="dot"))
        ...
        try:
            ...
            total_predicted = int(
                final_df[future_mask][C.UI_LABEL_CONTRACTS].sum())
            total_historical = len(signed_df)
            total_final = total_historical + total_predicted

            m1, m2 = st.columns(2)
            m1.metric(
                label=f"{C.UI_LABEL_NEW_CONTRACTS} ({horizon_label})", value=total_predicted)
            m2.metric(
        ...
        if run_bt_f:
            ...
            try:
                ...
                if "error" in bt_results:
                    ...
                    b1, b2, b3 = st.columns(3)
                    b1.metric("MAE (Erro Médio Absoluto)",
                              f"R$ {bt_results['mae']:,.2f}")
                    b2.metric("RMSE (Raiz do Erro Quadrático)",
                              f"R$ {bt_results['rmse']:,.2f}")
                    b3.metric("MAPE (Erro % Médio)",
                              f"{bt_results['mape']:.2f}%")
                    ...
                    comp_df = bt_results["comparison_df"]
                    fig_bt = px.line(title="Realizado vs Previsto (Backtest)")
                    fig_bt.add_scatter(x=comp_df[C.COL_INT_DATA], y=comp_df[f"{C.COL_INT_VALOR}_actual"], name="Realizado", line=dict(
                        color=C.COLOR_PRIMARY))
                    fig_bt.add_scatter(x=comp_df[C.COL_INT_DATA], y=comp_df[f"{C.COL_INT_VALOR}_predicted"], name="Previsto (Backtest)", line=dict(
                        color=C.COLOR_SECONDARY, dash="dot"))
                    st.plotly_chart(fig_bt, width="stretch")
